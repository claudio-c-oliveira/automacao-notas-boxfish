...
**Decisão de rollout (29/08, pedido da Michelle) — destino TEMPORÁRIO do arquivamento por função (Salvamento 2) em produção**: enquanto a Fase 2 estiver em validação (Michelle sobrecarregada, quer conferir o resultado antes de confiar 100%), o arquivamento por função NÃO deve escrever direto no Drive oficial do financeiro (`financeiro1@novorealitybox.com`) mesmo já em `ambiente = produção`. Em vez disso, deve escrever numa estrutura espelhada (mesmas subpastas `FINANCEIRO`/`FINANCEIRO S02`, mesmo padrão de nomenclatura) dentro do PRÓPRIO Drive pessoal da Michelle (`michelle.mimiaguia@gmail.com`) — ainda não existe, precisa ser criada. Motivo: com o volume de notas chegando e a automação processando rápido, um erro de arquivamento poderia passar despercebido e ser notado primeiro pela Controller (Danielle) reclamando com a Michelle — melhor a Michelle conferir no próprio Drive antes de copiar manualmente pro Drive oficial do financeiro. **Esta regra é temporária**: quando a Fase 2 estiver validada e confiável, o destino volta a ser o Drive oficial do financeiro diretamente, sem esse passo intermediário. Não confundir com a simulação de homologação já existente (pasta `HOMOLOG_BOX-FISH` no Drive pessoal do CLAUDIO, seção 7.6) — aquela é só para testes em ambiente homolog; esta nova pasta é para uso real em produção, temporário, no Drive da MICHELLE.
