...
- 1º Verificar Status do Contrato (coluna N da aba Notas). Essa checagem vale IGUALMENTE para os dois gatilhos abaixo ("PROGRAMAR ATÉ" e "SOLICITAR") — não é mais exclusiva do primeiro.
  ...
  - **Busca de ficha cadastral — REDESENHADA em 11/09, corrige bug crítico de falso positivo por correspondência de nome (caso real: "Vanessa Paes")**: a versão anterior buscava o nome do colaborador em qualquer lugar da mensagem (incluindo CC), o que confirmava "OK - ASSINADO" com base em alguém apenas copiado numa thread alheia — ex.: a Coordenadora Executiva, que fica em cópia em quase toda thread de contratação da empresa, casava com sua própria linha de colaboradora (mesma pessoa, dois papéis) sem a ficha ser dela. A nova lógica tem DUAS ETAPAS distintas, com papéis diferentes — a primeira nunca decide sozinha, só restringe a lista de candidatos; só a segunda confirma:
    ...
    - **Etapa 2 — Confirmar de verdade, lendo o conteúdo do anexo (usa IA)**: para cada candidato que sobrou da Etapa 1, abrir o(s) anexo(s) da mensagem (sempre PDF ou Word, nunca imagem/escaneado — confirmado por Claudio) e extrair o texto. Confirmar "OK - ASSINADO" SOMENTE se o nome do colaborador OU a razão social da empresa (o mesmo par de dados da planilha Cost) aparecer DENTRO do conteúdo do documento. Isso é o que garante que a ficha pertence de fato à pessoa/empresa certa — não só que a mensagem "parece" ser dela. Esta etapa precisa de IA (extração de texto de PDF/Word + comparação de nome com a mesma tolerância a variação de grafia já definida para a checagem de Contratos, seção acima) — revoga a decisão anterior de manter essa busca "sem IA": aquela decisão fazia sentido quando o critério era só remetente+anexo (padrão fixo, sem ambiguidade), mas deixou de se sustentar quando a confirmação real depende de interpretar o conteúdo de um documento.
...
PENDENTE (encontrado em 09/09, BLOQUEIA o primeiro disparo real em produção): a credencial `Claude account (produção)` NÃO existe no n8n — só existe `Claude account (dev)`. O workflow de produção amarra o node de IA (parsing da coluna J, seção 3.2) a esse nome exato; sem ela, falha na primeira linha processada. Ação de Claudio: criar a credencial no n8n com esse nome exato, apontando para uma chave de API real (pode reusar a mesma chave da Anthropic da conta dev, ou segregar — decisão de organização, não exigência técnica).
