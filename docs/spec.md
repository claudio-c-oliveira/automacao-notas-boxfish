...
- 1º Verificar Status do Contrato (coluna N da aba Notas). Essa checagem vale IGUALMENTE para os dois gatilhos abaixo ("PROGRAMAR ATÉ" e "SOLICITAR") — não é mais exclusiva do primeiro.
  ...
  - **Busca de ficha cadastral — REDESENHADA em 11/09, corrige bug crítico de falso positivo por correspondência de nome (caso real: "Vanessa Paes")**: a versão anterior buscava o nome do colaborador em qualquer lugar da mensagem (incluindo CC), o que confirmava "OK - ASSINADO" com base em alguém apenas copiado numa thread alheia — ex.: a Coordenadora Executiva, que fica em cópia em quase toda thread de contratação da empresa, casava com sua própria linha de colaboradora (mesma pessoa, dois papéis) sem a ficha ser dela. A nova lógica tem DUAS ETAPAS distintas, com papéis diferentes — a primeira nunca decide sozinha, só restringe a lista de candidatos; só a segunda confirma:
    - **Etapa 1 — Restringir candidatos (sem abrir anexo, rápido e barato)**: dentro do marcador "CONTRATAÇÃO", buscar mensagens com `has:attachment` e remetente de FORA dos domínios internos da empresa (hoje: `novorealitybox.com` e `boxfishtv.com` — lista pode crescer, checar com Claudio se aparecer um terceiro domínio). Dentro desse conjunto, um candidato é "plausível" se: (a) a thread foi endereçada originalmente para o e-mail do colaborador que está na coluna R da planilha Cost, OU (b) o nome do colaborador OU a razão social da empresa (ambos já existentes na planilha Cost) aparece no ASSUNTO ou no CORPO NOVO da mensagem de resposta (nunca no CC, nunca em corpo citado/thread anterior, nunca na lista de destinatários). Esses dois critérios servem só para reduzir a lista de e-mails a investigar (de centenas para um punhado) — não são suficientes sozinhos para confirmar o contrato.
    - **Etapa 2 — Confirmar de verdade, lendo o conteúdo da ficha cadastral (SEM IA — corrigido em 11/09 após investigação real)**: investigação nos anexos reais mostrou que a premissa inicial ("sempre PDF ou Word") estava errada — a distribuição real é 29 PDF, 8 xlsx, 2 docx, 2 html, 1 jpeg por mensagem, mas **a ficha cadastral em si é sempre um arquivo `.xlsx`** (confirmado por Claudio; exemplo real: `BR_SMTC_S01_FICHA CADASTRAL....xlsx`). Os PDFs anexados são documentos de APOIO da empresa contratada (certidão de NIRE, CNPJ, contrato de locação) — provam que a empresa existe e está regular, mas NÃO são o documento usado para confirmar identidade, e não entram nesta checagem. Etapa 2 revisada: entre os anexos do candidato (vindos da Etapa 1), identificar o arquivo `.xlsx` (a ficha cadastral real — pode-se usar "FICHA" no nome do arquivo como sinal adicional). Ler esse arquivo com `exceljs` (já disponível na VM, mesma biblioteca usada para Cost Report e Contratos — SEM custo de IA, sem PDF, sem rebuild de imagem) e aplicar a MESMA lógica de correspondência tolerante já definida para a checagem de Contratos (documento > nome com tolerância de 1º/último token > ambiguidade cai em alerta) comparando o conteúdo da ficha com o nome do colaborador / razão social da planilha Cost. Confirmar "OK - ASSINADO" somente com esse match positivo. **Anexos que não são `.xlsx`** (só PDF, Word, imagem, HTML) não são suficientes para confirmar — a linha cai em pendente, mesmo que a Etapa 1 tenha achado um candidato plausível; não há suporte hoje para ler Word/imagem como ficha (raro: 2/42 nos dados reais), fica para decisão futura se a demanda justificar.
    - **Se nenhum candidato da Etapa 1 tiver Etapa 2 confirmada**: a linha fica PENDENTE — mesmo que algum candidato pareça "quase certo" pela Etapa 1, nunca liberar só com base nela. Não adivinhar; melhor esperar confirmação manual da Michelle do que liberar pedido de nota para pessoa/empresa errada.
...
**Processamento SEQUENCIAL por fonte de Cost Report, nunca misturado (decidido em 09/09 — corrige bug crítico encontrado antes do primeiro disparo real em produção)**: quando a execução processa mais de uma fonte de Cost Report na mesma rodada (produção tem duas: S01 — AREP+Reunion, e S02 — Soft Pré), o ciclo completo de leitura → checagem de contrato → escrita → confirmação (as duas camadas acima) deve rodar por INTEIRO para uma fonte antes de começar a próxima — nunca as duas fontes abertas/escritas/conferidas juntas na mesma passada. Cada fonte usa seu próprio caminho de planilha de Contratos para a checagem: lendo S01, busca em `JURIDICO > BR_AREP_S01_REUNION_CONTROLE DE CONTRATOS.xlsb` (Reunion) e `JURIDICO > BR_AREP_S01_CONTROLE DE CONTRATOS.xlsb` (AREP); lendo S02, busca em `SMTC_S02_EXECUTIVA > JURIDICO S02 > BR_SMTC_S02_CONTROLE DE CONTRATOS.xlsb` (Soft Pré). Motivo: como os números de linha se repetem entre S01 e S02 (ex. célula N293 existe nas duas planilhas, são arquivos independentes), tentar escrever/conferir as duas fontes na mesma operação fazia a Camada 2 (arquivo salvo) conferir a lista de mudanças de AMBAS as fontes contra um único arquivo aberto por vez — na melhor hipótese abortando a execução inteira por divergência espúria, na pior (e mais perigosa) dando falso positivo: confirmando como bem-sucedida uma escrita que nunca aconteceu na outra fonte, por coincidência de valor na mesma célula/linha. Processar uma fonte inteira de cada vez elimina esse cenário — a conferência de cada fonte só compara contra a lista de mudanças daquela mesma fonte, nunca uma mistura.
...
**Detalhes de implementação do caso "COLABORADOR, 2 PERÍODOS, MESMO VENCIMENTO" (confirmados em 27/08):**
- **Agrupamento**: as duas linhas (mesmo projeto + colaborador + vencimento) formam UM ÚNICO e-mail com dois blocos — não dois e-mails separados. Colaboradores diferentes, mesmo que no mesmo vencimento, continuam gerando e-mails separados (regra normal).
- **Dois cenários distintos que resultam em "REF. MÊS1 E MÊS2", não confundir**: (a) DUAS LINHAS separadas na planilha, cada uma com seu próprio período dentro de um mês, mesmo vencimento — vira dois blocos numerados "1-"/"2-" no e-mail, como descrito nesta seção; (b) UMA ÚNICA LINHA cujo período (coluna J) atravessa dois meses (ex. "26/07 A 05/08") — continua sendo UM bloco só (não numera), mas o REF. do assunto ainda cita os dois meses, porque o período em si cruza a virada do mês.
- **Mês de referência quando um período (dentro de uma única linha, coluna J) atravessa dois meses** (ex.: "26/07 A 05/08"): o REF. do assunto cita OS DOIS MESES, no mesmo formato "[MÊS1] E [MÊS2]" já usado para o caso de duas linhas — NÃO escolhe um mês só (nem pelo mês da primeira data, nem por quantidade de dias trabalhados em cada mês; critério é puramente sobre o período cronológico do colaborador, não sobre carga de trabalho). *(Corrigido em 27/08 — versão anterior desta seção dizia "usar o mês da primeira data", que estava errada; o correto é citar ambos.)*
- **Normalização de ordem cronológica (novo, 27/08)**: tanto o período exibido no corpo do e-mail quanto a ordem dos meses no REF. devem estar em ordem cronológica ASCENDENTE (data mais cedo primeiro), independentemente de como a data está escrita na planilha de origem. Se a coluna J trouxer as datas invertidas por erro de digitação (ex.: "05/08 A 26/07"), a automação corrige e exibe "26/07 A 05/08" — nunca reproduz uma ordem de datas invertida vinda da planilha.
- **Ordem dos meses no assunto** ("REF. [MÊS1] E [MÊS2]"): segue a ORDEM DE APARIÇÃO das linhas na planilha, não ordem cronológica ascendente — evita inverter um par que atravessa a virada do ano (ex.: DEZ + JAN não pode virar "JAN E DEZ").
- **Texto de abertura do e-mail no plural quando há mais de um bloco**: "para a emissão da sua nota fiscal" (singular, usado quando há só 1 bloco) deve virar "para a emissão das suas notas fiscais" (plural) quando o e-mail tiver mais de um bloco — pluralizar só os termos necessários ("nota fiscal" → "notas fiscais", "sua" → "suas"), sem alterar o resto da frase.
...
**Decisão de rollout (29/08, pedido da Michelle; regra fechada em 07/09, PASTA AINDA NÃO CRIADA — bloqueada pelas credenciais Google expiradas, ver seção 9) — destino do arquivamento por função (Salvamento 2) em produção depende do MODO_EXECUCAO**: com `ambiente = produção`, o destino do Salvamento 2 segue o mesmo padrão já usado para e-mail (rascunho vs. envio direto, seção 3.1.3) — não é uma janela temporária avulsa, é regido pelo `MODO_EXECUCAO`:
- **`MODO_EXECUCAO = rascunho`**: escreve na pasta raiz **`BOX-FISH_ARQUIVO-POR-FUNCAO_ATIVA`** (ID `1J4aNIZ2hpnXwGYPVCPI2LVPi4T1kySXH`), dentro do Drive pessoal da Michelle (`michelle.mimiaguia@gmail.com`) — mesmas subpastas `FINANCEIRO`/`FINANCEIRO S02`, mesmo padrão de nomenclatura da estrutura oficial. Michelle confere o resultado ali antes de copiar manualmente pro Drive oficial. **Correção de nome (11/09)**: a pasta original `PRODUCAO_BOX-FISH_A-CONFERIR`, criada manualmente por Claudio pelo navegador, ficou OBSOLETA e SEM USO — ela nunca é acessível por nenhuma credencial de app (Michelle ou produção), porque o escopo OAuth do projeto (`drive.file`, o mesmo problema que bloqueou o Cost Report de homolog em 07/09) só enxerga pastas criadas pelo próprio app, nunca as criadas manualmente no navegador. A pasta nova (`BOX-FISH_ARQUIVO-POR-FUNCAO_ATIVA`) foi criada pelo próprio Claude Code, então é acessível. Não usar mais a pasta antiga para nada; ela pode ser apagada manualmente por Claudio quando conveniente (o app não consegue nem renomeá-la/apagá-la, pelo mesmo motivo de escopo).
- **`MODO_EXECUCAO = automatico`**: escreve direto no Drive oficial do financeiro (`financeiro1@novorealitybox.com`), estrutura real `FINANCEIRO > 08. NOTAS ARQUIVADAS > ...` — sem passo intermediário.
Motivo do modo rascunho ir pro Drive da Michelle: com o volume de notas chegando e a automação processando rápido, um erro de arquivamento poderia passar despercebido e ser notado primeiro pela Controller (Danielle) reclamando com a Michelle — melhor ela conferir no próprio Drive antes de confiar no automático. Não confundir com a simulação de homologação já existente (pasta `HOMOLOG_BOX-FISH` no Drive pessoal do CLAUDIO, seção 7.6) — aquela é só para testes em ambiente homolog; `BOX-FISH_ARQUIVO-POR-FUNCAO_ATIVA` é para uso real em produção, no Drive da MICHELLE. **Esclarecimento: esta regra (ligada ao MODO_EXECUCAO) é EXCLUSIVA do Salvamento 2 (arquivamento por função). O Salvamento 1 (pouso de trabalho, por retranca/projeto, seção 4.2) NÃO muda — sempre vai para a pasta `SMTC` (produção) ou `HOMOLOG_BOX-FISH` (homolog) dentro do Drive da Michelle, independente do MODO_EXECUCAO, como já era.**
...
- **Desambiguação quando o mesmo fornecedor tem múltiplas linhas na planilha (decidido em 11/09)**: um fornecedor pode aparecer em várias linhas com códigos/cargos diferentes (ex.: uma agência de viagens com linhas separadas para "PASSAGEM", "HOSPEDAGEM"; uma produtora com "DIREÇÃO TÉCNICA" e "COORDENAÇÃO TÉCNICA"). Nesse caso, o nome do arquivo da nota (razão social) sozinho não basta — usar o seguinte filtro em cascata, do mais amplo ao mais específico: (1) filtrar a planilha Cost pelo fornecedor (razão social); (2) dentro desse subconjunto, filtrar pela data de vencimento (coluna G), usando a data que já vem da própria estrutura de pastas de origem (organizada por vencimento); (3) se ainda houver mais de uma linha candidata, casar pelo NÚMERO DA NOTA FISCAL (coluna E, "Documento Fiscal") — que já vem no próprio nome do arquivo da nota, depois do "#" (ex. "..._#000123.pdf" → "000123") — contra o valor da coluna E de cada linha candidata; o número da nota é um identificador exato e único, diferente de valor monetário (que pode coincidir entre notas diferentes). Esse mesmo princípio (fornecedor + vencimento + número do documento) deve ser usado pela Fase 2, no futuro, para resolver o "avulso" quando o fornecedor/colaborador tiver múltiplas linhas na planilha — não usar só correspondência de nome/razão social nesse cenário.
- **Normalização do número ao comparar com a coluna E — AUTORIZADA (11/09)**: faturas de empresa (diferente de nota de colaborador) frequentemente têm numeração própria com hífen (ex. "14-9"), diferente do padrão de 6 dígitos sem separador usado nas notas de colaborador. Ao gerar o nome do arquivo (retranca), a Michelle converte esse número pro padrão de 6 dígitos removendo o hífen e completando com zeros à esquerda (ex. "14-9" → "000149"). Por isso, ao comparar o número extraído do nome do arquivo com o valor da coluna E, remover zeros à esquerda e hífens dos dois lados antes de comparar (ex. "000149" e "14-9" → ambos viram "149", batem). Isso NÃO é uma aproximação arriscada — é reconhecer a mesma informação escrita em dois formatos diferentes por convenção conhecida, não uma tentativa de casar valores parecidos que poderiam ser documentos diferentes.
- Padrão de nome de pasta por função — confirmado com exemplos reais: AREP usa "[código] - [CARGO]" sem sufixo (ex.: "1301 - DIRETOR GERAL", "2011 - ..."); Reunion usa "[código].R - [CARGO]" (ex.: "1301.R - DIRETOR GERAL", "7007.R - ACESSORIA JURÍDICA", "2794.R - VERBA FIGURINO REUNION"); Soft Pré usa "[código].S - [CARGO]" (ex.: "3101.S - DIRETOR DE FOTOGRAFIA", "3428.S - PRODUTORA DE CASTING") — **corrigido em 11/09**: a suposição anterior de que Soft Pré seguiria o padrão do AREP sem sufixo estava errada; o dado real da coluna B da planilha Cost já vem com ".S" — usar sempre o valor literal da coluna B, não inferir o sufixo.
...
- **Frequência do disparo automático em produção — decidido em 27/08, horário fechado em 29/08**: de 3 em 3 horas, das 9h às 21h, dias úteis (seção acima). Horários de disparo: 9h, 12h, 15h, 18h, 21h. Contexto da decisão: "SOLICITAR URG" não exige resposta em tempo real — o gargalo real está no colaborador enviar a nota depois de receber o pedido, não na velocidade da Michelle solicitando; "urgente" aqui significa "processar ainda no mesmo dia", não "imediatamente". Por isso um intervalo de poucas horas (não minutos, não hora em hora) já atende a urgência real do processo.
...
- **Esclarecido em 09/09**: o limite é um TETO (máximo), não uma cota fixa/exata — se um projeto tiver menos linhas elegíveis do que o limite, processa todas as que existirem, sem forçar chegar no número. E o teto é aplicado **por PROJETO** (AREP, Reunion, Soft Pré cada um com seu próprio teto independente), não por arquivo — importante porque AREP e Reunion compartilham o mesmo arquivo físico (Cost Report S01, diferenciados por cor de linha, seção 6): um teto "por arquivo" faria os dois disputarem a mesma cota; o teto "por projeto" garante que cada um seja avaliado separadamente. Exemplo: com limite=20, se AREP tiver 2 linhas elegíveis e Reunion tiver 15, as duas rodam por inteiro (2 e 15, respectivamente) mesmo estando no mesmo arquivo — nenhum disputa vaga com o outro.
...
RESOLVIDO (10/09): a credencial `Claude account (produção)` foi criada no n8n e testada com sucesso.
MARCO (11/09): primeira execução real da Fase 1 em produção rodando de ponta a ponta com sucesso (execuções 276-278, via despachante por fonte, S01 e S02). Escrita confirmada na planilha real (S02, linha 13, coluna N = "OK - ASSINADO"), lock liberado corretamente, dedup funcionando com dado real (rascunho já existente reconhecido, sem duplicar), checagem de ficha cadastral corretamente não confirmando por falta de prova documental (foi a planilha de Contratos que liberou) — validação prática da correção do caso "Vanessa Paes".
PENDENTE (decisão de padronização, não bloqueante): a automação escreve "OK - ASSINADO" (com hífen, conforme o spec), mas as linhas preenchidas manualmente pela Michelle/Danielle usam "OK ASSINADO" (sem hífen). Visualmente destoa na planilha, e um filtro manual que compare texto exato pode não pegar as linhas da automação. Recomendação: manter o padrão da automação (com hífen) e, se necessário, padronizar manualmente as linhas antigas — não mudar a automação para bater com uma inconsistência pré-existente.
...
DECIDIDO (07/09, resolve o risco crítico encontrado nesta data): os arquivos de configuração (`cobrancas.json`, `apelidos.json`, `config_execucao.json`) passam a ser SEPARADOS por ambiente, em vez de um único arquivo compartilhado. Convenção de nome: arquivo sem sufixo = produção (ex. `cobrancas.json`); arquivo com sufixo `_hml` = homologação (ex. `cobrancas_hml.json`). Motivo: a planilha de teste de homolog é uma cópia real da planilha Cost (mesmos números de linha), então um arquivo único fazia a deduplicação da seção 3.1.6 correr o risco de tratar linhas reais de produção como "já processadas" só porque a mesma linha foi tocada num teste de homolog. **Sem necessidade de migração dos dados atuais**: a exigência de "nunca perder um ID já contabilizado" vale só para dados de PRODUÇÃO daqui pra frente (depois que execuções reais começarem) — os dados que hoje existem no `cobrancas.json` são todos de testes de homolog, então não precisam ser preservados na separação. Ambos os arquivos (`cobrancas.json` de produção e `cobrancas_hml.json` de homolog) podem nascer vazios. **Extensão (07/09): a mesma separação por ambiente (sufixo `_hml`) vale também para `log_diario.json` e `pendencias_identificacao.json`** — mesmo mecanismo (`__SUFIXO_AMBIENTE__`), mesma regra de dados existentes (preservar o que já existe movendo pro arquivo `_hml`, já que são dados de homolog; produção nasce vazio).
PENDENTE (encontrado em 27/08, seção 3.1.8): quando a Michelle envia manualmente um e-mail de "PROGRAMAR ATÉ" e avança o Status Box (ex. para "ENTREGUE MI") antes da automação agir na data-alvo, a entrada correspondente do rascunho fica órfã no `cobrancas.json` — nunca é limpa, porque a linha não bate mais no filtro de Status Box e nunca mais é processada. Não causa dano imediato (só cresce o arquivo), mas é dívida técnica — precisaria de uma rotina própria de limpeza de estado órfão. Não implementado, sem prioridade definida ainda.
PENDENTE (levantamento de 07/09) — estado real da Fase 2: NÃO é esqueleto vazio, tem lógica substancial já implementada nos 3 arquivos (`fase2_recebimento_validacao.json` 35 nodes, `fase2_verificar_pendencias.json` 28 nodes, `fase2_finalizar_nota.json` 22 nodes) — incluindo monitoramento de e-mail com anexo, extração de NF via IA, match de colaborador via IA, atualização de colunas D/E/F e "RECEBIDA MI", e o ciclo de correlação por reply no Telegram (janela 8h/26h) exatamente como especificado. MAS nenhum dos 3 está seguro para publicar — 5 bloqueadores encontrados: (1) `boxFileId` nunca é definido, usado em 5 pontos (lock/download/upload/unlock); (2) 4 IDs de pasta do Drive são literais `REPLACE_WITH_FOLDER_ID_…`; (3) MAIS GRAVE — perda de contexto depois do node de Gmail "Baixar anexo": o `$json.config.ambiente` deixa de existir logo depois, fazendo a checagem de ambiente cair sempre no ramo de produção (usaria a credencial de produção da IA mesmo em homolog) — e a extração da IA sobrescreve campos (`ehConhecida`, `roster`, `perfis`) que nodes seguintes dependem; (4) busca manual de e-mails sem `simple: false`, fazendo o filtro descartar 100% dos e-mails naquele ramo; (5) nenhum ramo por ambiente para o Cost Report — os 3 downloads vão direto ao Box de PRODUÇÃO, então rodar a Fase 2 em homolog hoje leria/escreveria (lock+upload) na planilha real. Além disso, a regra de Salvamento 2 por MODO_EXECUCAO (seção 4.2) ainda não existe no código da Fase 2 — só no spec. Os 3 arquivos também estão fora da arquitetura de fonte única (seção 7.6) e usam nomes de config sem sufixo de ambiente, então apontam pros arquivos de produção em qualquer ambiente — não é risco imediato (nenhum publicado), mas é o primeiro item a resolver quando entrarem na migração de fonte única. Recomendação: não publicar nenhum dos 3 até esses bloqueadores serem corrigidos.
